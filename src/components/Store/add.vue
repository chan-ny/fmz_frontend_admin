      ...
      <v-card>
        ...
        <v-card-actions>
          <v-btn dark small color="orange" @click="dialogForm = false"
            >ຍົກເລີກ</v-btn
          >
          <v-btn small color="success" @click="addStore">ບັນທືກ</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
    ...
    async addStore() {
      ...
      if (this.$v.$invalid) {
        ...
        try {
          ...
          let save = await StoreService.addstore(format, config);
          ...
          Notify.showMessage(
            "error",
            "ຂໍ້ມູນບໍ່ສາມາດບັນທືກ ກະລຸນາລອງໄໝ່",
            3000
          );
        }
      }
