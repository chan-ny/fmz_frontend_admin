<template>
  <div>
    <v-btn class="ml-2" color="success" dark tile elevation="5" @click="dialogfrom = true"
    ...
    >
      <v-card>
        <v-card-title>
          <span class="headline">
            <div class="font">ເພີ່ມຂະໜາດສິນຄ້າ</div>
          </span>
        </v-card-title>
        <v-divider></v-divider>
        ...
        <v-card-actions>
          ...
          <div>
            <v-btn color="green darken-1" dark @click="addSize">
              ບັນທຶກ
            </v-btn>
          </div>
        </v-card-actions>
  ...
  methods: {
    ...
    async addSize() {
      ...
      if (this.$v.$invalid) {
        ...
        try {
          let save = await SizeServiec.create({
            "sname": this.sizeName,
          ...
          Notify.showMessage("error", "ບໍ່ສາມາດບັນທືກຂໍ້ມູນຂະໜາດສິນຄ້າ", 4000);
        }
      }
    }
  },
